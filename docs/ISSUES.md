...
- 상태: **해결됨 (21일차)** — 본문이 "있어야 한다"고 적은 부팅 루틴을 실제로 만들었다
...
**해소**: 21일차. 위 본문이 "있어야 한다"고 적은 그 부팅 루틴을 만들었다.

- `lib/storage/temp-cleanup.ts`의 `cleanupOrphanTempFiles(root)` — `root` 아래를 재귀로 훑어 orphan
  임시 파일을 지우고 지운 경로를 돌려준다. **절대 던지지 않는다** — 서버 부팅이 흘러간 임시 파일 하나
  때문에 멈추면 그게 더 나쁜 고장이다. `root`를 인자로 받는 것은 테스트가 실제 `data/`를 건드리지
  않게 하기 위함이다.
- `instrumentation.ts`의 `register()`가 부른다. 설치본 문서가 "called **once** when a new Next.js
  server instance is initiated"라고 못박은 자리이고(`01-app/03-api-reference/03-file-conventions/
  instrumentation.md`), 같은 문서가 지시하는 `NEXT_RUNTIME === 'nodejs'` 가드와 동적 import를 지킨다.
  0건일 때는 아무 로그도 남기지 않는다.

**`*.tmp`를 전부 지우지 않는다.** `atomicWriteFile`이 만드는 이름 형태(`<원본경로>.<uuid v4>.tmp`)와
uuid까지 일치하는 파일만 지운다. `data/`는 사용자가 손으로 열어 보는 디렉터리이고(그래서 JSON 파싱
실패를 조용히 덮어쓰지 않는다 — CONVENTIONS §7), 거기 둔 개인 파일을 부팅할 때마다 말없이 지우는
도구가 되면 안 된다.

**실측**: `data/`에 orphan 형태 파일과 `내메모.tmp`를 나란히 놓고 `next start`를 띄웠다. 로그에
`[instrumentation] 남아 있던 임시 파일 1건을 정리했습니다.`가 찍히고 orphan만 사라졌으며 `내메모.tmp`는
그대로 남았다. 단위 테스트는 `lib/storage/temp-cleanup.test.ts`(7건 — 중첩 디렉터리·정상 파일 보존·
우리가 만들지 않은 이름 4종·디렉터리 부재).
...
- 상태: **해결됨 (21일차)** — 20일차에 문구를 정정했고(부분 해결), 21일차에 본문이 "고친다면 볼 곳"으로
  지목한 스키마 정합까지 끝냈다. 아래 본문은 발견 당시 그대로 남긴다
...
**해소**: 21일차. 위 "고친다면 볼 곳" 중 **두 번째 방향**(구분 필드 신설)을 택했다. 첫 번째 방향
(`failCount`의 단위를 언론사에 맞추기)은 버린다 — 수집 결과 화면(`run-summary-card.tsx`)과 실행 목록
라벨이 이미 `failCount`를 기사 단위로 정확히 쓰고 있어, 단위를 바꾸면 **지금 맞는 화면 두 곳이 틀려진다.**

- `CrawlRun.failedPressCount` 신설(선택 필드 + 기본값 0 — `skippedCount`·`pressResults`와 같은 이유로,
  필수로 두면 과거 `run-meta.json`이 스키마에서 떨어져 그 run이 목록에서 통째로 사라진다).
  `finishRun`이 받아 파일에 남긴다. **`status` 판정 기준은 바꾸지 않았다** — 기사 1건이라도 실패했으면
  그 실행은 실제로 "일부 실패"가 맞다. 두 숫자는 서로를 대체하지 않고 각자의 단위로 쓰인다.
- `RunProgress.failedPressCount`는 **선택 필드**이고, `undefined`가 "0"이 아니라 **"이 스냅샷으로는 알 수
  없다"**를 뜻한다. 근사 복원 경로에서만 그렇게 온다.
- `run-manager.ts`는 판정을 새로 하지 않고 `isTotalFailure`가 이미 찍어 둔 `status === 'failed'`를 셀
  뿐이다(`countFailedPresses`). 같은 판정을 두 곳에서 하면 한쪽만 고쳐지는 사고가 난다.

**이번에 실제로 닫힌 거짓말은 20일차가 못 잡은 쪽이다.** 20일차 수정은 화면이 `pressStatuses`에서
직접 세는 방식이었는데, **근사 복원 경로에서는 실패한 언론사가 `'waiting'`으로 보여 세어 보면 언제나
0곳이 나온다.** 그래서 서버 재시작 뒤 그 실행을 조회하면 실제로 언론사가 실패했는데도 화면이 "언론사는
모두 정상 처리됐지만…"이라고 단정했다. 이제 그 경로는 `undefined`를 받아 **단위를 단정하지 않는 세 번째
문구**를 쓴다. `docs/screens/01-crawl-run.md` §⑤에 세 갈래 표로 반영했다.

**회귀 테스트**: `run-manager.test.ts` 2건(근사 복원은 `undefined`로 남긴다 · `pressResults` 경로는 파일
값이 아니라 목록에서 다시 세어 화면과 어긋나지 않게 한다) + `run-repository.test.ts` 3건(저장·`status`
기준 불변·과거 파일 호환). 기존 I-017 테스트에 `failedPressCount: 0` 어서션을 더했다 — **기사 1건 실패 +
언론사 0곳 실패**라는 이 이슈의 핵심 상황이 그 테스트에 이미 들어 있었다.

**여전히 실물 재현은 못 했다.** 특정 기사 URL만 골라 실패시키려면 언론사 서버 응답에 의존해야 한다 —
16일차와 같은 이유다. 위 경로들은 단위 테스트로만 확인했다.
...
- 상태: **해결됨 (21일차)** — 본문 "고친다면"이 지목한 단일 스캔 구조로 바꿨다
...
**해소**: 21일차. 위 "고친다면" 그대로 했다 — 네 개의 `.replace()`를
`/&(?:#x([0-9a-f]+)|#(\d+)|(nbsp|lt|gt|quot|apos|amp));/gi` 하나 + 단일 `.replace()` 콜백으로 합쳤다.
`&amp;`도 `NAMED_ENTITIES` 표로 들어왔다 — 스캔이 한 번이 되면서 "항상 마지막에 처리한다"는 순서
규칙 자체가 필요 없어졌다.

**회귀 테스트**: `lib/crawler/rss.test.ts`에 7건 추가. 본문 표의 여섯 케이스를 `it.each`로 그대로 태우고
(`&#38;apos;` → `&apos;` 등), 16진·10진·이름 있는 엔티티가 한 문자열에 섞인 경우도 각각 한 번씩만
풀리는지 확인한다. 기존 `&amp;apos;` 테스트(반대 방향)는 그대로 두고 주석만 고쳤다 — 그 방향은
"마지막에 처리"로 막고 있었을 뿐 반대 방향이 뚫려 있었다는 사실이 이 이슈의 요지이므로, 두 방향이
한 파일에 나란히 있는 편이 다음 사람에게 정확하다.
...
**22일차 갱신 — 표본이 두 배가 되며 부담도 두 배가 됐다**

- 발견: 22일차 · 크롤 파이프라인(Task 030 — 언론사 표본 확충, 사용자 요청 "정확한 분석을 위해 언론사를
  많이 추가해줬으면 좋겠다")
- 관련 이슈: **I-054**(`docs/ISSUES.md` — "완전 초기 상태에서 언론사가 0건이라 사용자가 십수 곳을 손으로
  입력해야 한다", 21일차 발견 · 보류 상태 유지 중)
- 관련 Task: Task 030(이 영역) · Task 009(화면, 빈 상태 설계 원출처) · Task 006(저장소 계층,
  `press-defaults.ts`·`press-repository.ts` 소유)

**무슨 일이 있었나**: Task 030으로 5개 카테고리 각각 최소 6곳(총 34곳)까지 표본을 늘렸다. I-037의 빈 상태
설계(`DEFAULT_PRESS_SOURCES`는 빈 배열)는 이번 회차에도 그대로 지켰고, 등록은 전부 `POST /api/press` →
`data/press-sources.json` 경로로만 이뤄졌다 — 코드가 아니라 런타임 데이터만 늘었다.

**I-054가 남긴 논점이 이번 회차로 더 무거워졌다**: 21일차엔 "완전 초기 상태에서 17곳을 손으로 입력해야
한다"였는데, 이번 회차로 그 숫자가 **34곳**이 됐다. 시드로 풀지 가져오기 기능으로 풀지의 결정은 여전히
I-037의 빈 상태 설계를 지키는 선에서 화면 워크스트림과 함께 다음 회차에 검토해야 하는 문제이고, 이 메모는
그 결정을 미루는 동안 문제의 크기가 계속 커지고 있다는 사실만 기록해 둔다. 실물 후보 목록(URL·인코딩·
셀렉터 실측값)은 `docs/press-candidates.md`에 34곳 전부 남아 있으므로, 다음 회차에 시드 또는 가져오기
기능을 만들 때 새로 조사할 필요는 없다.

**해소**: 없음 — I-054의 보류 상태를 그대로 유지한다. 이 메모는 팀장이 `docs/ISSUES.md`의 I-054를 갱신할
때 참고할 근거 자료다.
...
### I-056 · 이중 인코딩 매체(아주경제 재테크)가 실제로 키워드 랭킹을 오염시켰다 — 실측 기준선

- 상태: **해결됨 (22일차)** — 매체를 서울파이낸스로 교체해 해소했다(**D-057**). 파서는 손대지 않았다
- 발견: 22일차 · 크롤 파이프라인(Task 029 교차검증 중 발견 → 팀장이 이번 회차 범위로 재지정)
- 관련 Task: Task 030(언론사 등록) · Task 010A(엔티티 디코딩, `lib/crawler/rss.ts` 소유)
- 관련 이슈: I-043(`docs/ISSUES.md` — "이중 이스케이프는 정확히 한 번만 스캔한다", 이 판정의 근거)

**무슨 일이 있었나**: 22일차에 등록한 아주경제 재테크(`ajunews-investment`) 피드가 raw 바이트
수준에서 `&amp;quot;`·`&amp;#39;`·`&amp;nbsp;`로 **이중 인코딩**돼 있었다(76개 아이템 중 60개,
79% 영향). 우리 디코더는 I-043 규칙대로 한 번만 스캔해 `&quot;`·`&#39;`를 리터럴 텍스트로
남겼고(설계대로, 버그 아님), 그 결과가 저장된 기사 txt와 키워드 랭킹까지 그대로 샜다.

**실측 기준선(다음에 같은 판단을 할 때 비교 기준)**: `runId: 20260811-085056`(아주경제 5건
포함, 전체 25건 중 20%)의 `GET /api/runs/20260811-085056/keywords` 응답에서
...
`quot`가 코스피(17회)보다 위인 4위, `nbsp`가 국내(13회)보다 위인 8위에 올랐다 — 매체 비중
20%만으로 상위 10위 안에 쓰레기 토큰 2개가 들어갈 만큼 오염 효과가 크다.

**해소**: 완료. `ajunews-investment`를 `DELETE`하고 서울파이낸스(`seoulfn-stock`)로 교체 —
등록 전 이중 인코딩 스캔(신규 절차, `docs/press-candidates.md` §재현 방법)을 통과했고, 교체 후
크롤(`runId: 20260811-092100`)의 저장 txt에서 엔티티 잔여물 0건을 확인했다. **에스컬레이션
기준**: 이중 인코딩 매체가 두 번째로 나타나면 개별 교체 대신 `lib/crawler/rss.ts`에 좁은 전용
전처리(`&amp;(quot|apos|lt|gt|nbsp|amp|#\d+|#x[0-9a-f]+);` 패턴 한정)를 추가하는 것을 검토한다
(`docs/DECISIONS.draft.크롤파이프라인.md`). 오염된 run(`20260811-085056`)은 증거로 보존한다.

### I-057 · ScrollArea + max-h 스크롤 결함, "flex-1 min-h-0"로는 안 고쳐진다 — grid로 전환해야 했다

- 상태: **해결됨 (22일차)** — `grid-rows-[minmax(0,1fr)]` + `min-h-0`으로 전환
- 발견: 22일차 · 화면(Task 029 — 수집 결과 검색어 하이라이트·목록 스크롤, 로드맵 밖 신규 요청)
- 관련 파일: `components/results/article-file-list.tsx`(수정) · `components/ui/scroll-area.tsx`(shadcn 생성물, 직접 수정하지 않음)
- 관련 컴포넌트: 다른 화면에서도 `ScrollArea`를 `max-h-*`와 함께 쓰는 곳이 있다면 같은 결함이 재현될 수 있다 —
  이번 회차에는 02(수집 결과)만 확인했고 03·04·05는 점검하지 않았다.

**무슨 일이 있었나**: Task 지시의 1차 제안("바깥을 `flex-col` + `max-h`로 감싸고 `ScrollArea`엔 `flex-1 min-h-0`를
준다")을 그대로 구현했다. `getComputedStyle`로 확인하면 바깥 wrapper와 `ScrollArea` Root의 높이는 정확히
`28rem`(448px)으로 잘 확정됐는데도, Radix의 내부 `Viewport`(`size-full` = `height:100%`)가 그 448px을 퍼센트로
resolve하지 못하고 계속 내용 전체 높이(예: 2280px)로 렌더링됐다(Playwright `getComputedStyle`/`scrollHeight`·
`clientHeight` 실측으로 확인). 결과적으로 스크롤이 여전히 안 됐다 — flex item이 flex 알고리즘으로 받은
"used height"가 그 자손의 퍼센트 높이 계산에서 항상 definite로 취급되는 것은 아니라는 뜻이다(브라우저 구현별로
갈리는 지점으로 보인다).

CSS Grid의 `grid-rows-[minmax(0,1fr)]` 트랙으로 바꾸자 곧바로 해결됐다 — grid의 fr 트랙이 자손의 퍼센트/
`h-full` 높이 계산에 훨씬 안정적으로 definite 크기를 전달한다. 최종 구현은
`hidden max-h-[28rem] grid-rows-[minmax(0,1fr)] lg:grid`(데스크톱)/
`grid max-h-[28rem] grid-rows-[minmax(0,1fr)] lg:hidden`(모바일)로 감싸는 바깥 `div`를 두고, `ScrollArea`엔
`min-h-0`만 준다(`min-h-0`이 없으면 grid item의 기본 최소 크기가 다시 내용만큼 트랙을 밀어 올린다).

**남기는 이유**: `components/ui/scroll-area.tsx`를 `max-h-*`와 함께 쓰는 다른 화면·컴포넌트가 있다면 같은
결함을 안고 있을 수 있다. "flex-1 min-h-0"는 이 컴포넌트 조합에서 통하지 않았다는 사실과 grid 대안을 남겨,
같은 시행착오를 반복하지 않게 한다.

**해소**: 이 영역(02-collect-result.md ④ 기사 파일 목록, 데스크톱·모바일 둘 다)은 grid 전환으로 해소했고
Playwright로 마우스 휠·스크롤바 드래그 양쪽이 끝 항목까지 도달하는 것을 실측 확인했다.

**추가 확인(팀장, 22일차)**: 다른 화면의 `ScrollArea` 사용처를 전수로 봤다 —
`components/crawl/press-run-status-list.tsx:56`(`h-[240px]`)·`components/crawl/press-select-card.tsx:147`
(`h-[320px] sm:h-[420px]`) 둘 다 `max-h`가 아니라 **처음부터 고정 높이**라 이 결함 자체가 성립하지 않는다.
`max-h` + `ScrollArea` 조합은 `/results`(02 화면)에만 있었다. **03·04·05는 점검 완료, 해당 없음 — 이 항목은
완전히 닫혔다.** 다음 회차에 재점검할 필요 없다.

---

### I-058 · 언론사 수가 늘면 화면 가정이 깨진다 — 카테고리 헤더가 스크롤에 그냥 흘러가던 문제 (Task 030 교차검증 발견 · 해소)

- 상태: **해결됨 (22일차)** — 카테고리 헤더에 `sticky top-0 z-10 bg-card` 적용
- 발견: 22일차 · 화면(Task 030 교차검증 중) — 크롤 파이프라인이 언론사 표본을 17→34곳으로 늘린(Task 030)
  직후 `/`(크롤링 실행) 화면을 실제로 스크롤해 보다가 발견했다.
- 관련 파일: `components/crawl/press-select-card.tsx`(수정) · `docs/screens/01-crawl-run.md`(반영)

**무슨 일이 있었나**: `press-select-card.tsx`의 언론사 목록은 카테고리별로 그룹핑돼 있고, 각 그룹 헤더
(`{카테고리} ({개수})` + 그룹 전체 선택 체크박스)는 `sticky` 없이 그냥 목록과 함께 스크롤됐다. 언론사가
17곳일 때는 카테고리당 3~4곳이라 그룹 하나가 `ScrollArea` 뷰포트(`h-[320px] sm:h-[420px]`)보다 작아 헤더가
스크롤 밖으로 나갈 일이 거의 없었다. Task 030으로 카테고리당 6곳 이상(스포츠 7·IT/AI 8 등)으로 늘자, 뷰포트
보다 큰 그룹이 여러 개 생겼고 — `scrollTop`을 950 근처로 두면(Playwright 실측) 화면에 "스포츠" 항목 6개가
보이는데 "스포츠 (7)" 헤더도 다음 "경제 (6)" 헤더도 둘 다 화면 밖이라 **지금 보고 있는 게 어느 카테고리인지
알 방법이 없었다.**

**왜 기록해 두는가**: 이 결함은 코드가 바뀌어서 생긴 게 아니라 **데이터(언론사 수)가 늘어서 비로소
드러났다.** 17곳일 땐 통과하던 화면 설계가 34곳에서 깨졌다는 것은, 다음에 언론사 표본을 또 늘리거나
(카테고리 상한을 더 올리거나) 다른 화면에서 비슷하게 "그룹 헤더 + 고정 높이 스크롤" 패턴을 쓸 때 **같은
자리를 다시 봐야 한다는 뜻**이다. "고정 높이라 스크롤 자체는 된다"와 "그 안에서 방향감을 잃지 않는다"는
서로 다른 조건이고, 표본이 작을 때는 후자가 시험되지 않는다.

**해소**: `press-select-card.tsx`의 카테고리 헤더에 `sticky top-0 z-10 bg-card`를 줬다(Radix `ScrollArea`의
Viewport가 실제 스크롤 컨테이너라 그 안에서 `sticky`가 그대로 성립한다). Playwright로 다음을 실측 확인했다.
- `scrollTop 950`(문제를 처음 관측한 지점)에서 "스포츠 (7)" 헤더가 뷰포트 상단에 고정돼 보인다.
- 그룹이 연달아 스크롤될 때 각 헤더의 `getBoundingClientRect()`가 순서대로 위→아래로 밀려나며 자연스럽게
  교체된다(별도 로직 없이 각 헤더가 자기 카테고리 `div`에 속한 `sticky` 요소라서 성립).
- 헤더가 고정된 상태에서 그룹 전체 선택 체크박스를 클릭해도 정상 동작한다(클릭 시 `7/33개 선택됨`으로
  갱신 확인).
- 라이트·다크 양쪽에서 `bg-card` 배경이 불투명하게 채워진다(`getComputedStyle`로 배경색 확인 +
  `elementFromPoint`로 헤더 전체 폭에서 DOM 오버랩이 없음을 확인).

`docs/screens/01-crawl-run.md`의 "카테고리 헤더 sticky 처리(22일차 후속)" 절과 영역별 컴포넌트 명세 표에
반영했다.

### I-059 · 목록에 자체 스크롤을 주면 그 영역에 닿기 위한 페이지 스크롤이 휠에 붙잡힌다

- 상태: **해결됨 (22일차 후속)** — 중첩 스크롤 컨테이너를 없애 페이지 스크롤 하나만 남겼다
- 발견: 22일차 마감 직후 · **사용자 실사용 보고**(팀장이 Playwright로 재현·수정)
- 관련 Task: Task 029(스크롤 처리를 만든 회차) · Task 030(언론사 확충으로 목록이 길어진 원인)
- 관련 이슈: **I-057**(같은 자리의 앞선 결함 — `max-h`만으로는 스크롤이 안 생기던 문제) ·
  **I-058**(같은 계열 — 언론사 수가 늘며 화면 가정이 깨졌다)

**증상**: `/results`에서 기사가 많이 쌓이면 **아래쪽 파일을 읽을 수도 클릭할 수도 없다.**
스크롤이 내려가지 않는다.

**I-057을 고친 것이 이 문제를 만들었다.** Task 029는 `ScrollArea`가 `max-h`만으로는 스크롤되지
않던 것을 `grid-rows-[minmax(0,1fr)]`로 고쳤고, `scrollTop`이 끝까지 도달하는 것까지 실측했다.
**그 검증은 맞았다** — 커서가 이미 목록 위에 있을 때는 정상 동작한다. 검증이 놓친 것은 **거기까지
가는 길**이다.

**측정값**(창 1440×900, 기사 507건):

| 값 | 실측 |
| --- | --- |
| ④ 기사 파일 카드가 시작하는 위치 | 페이지 최상단 기준 **677px** |
| 목록 스크롤 영역 시작 | **839px** |
| 창 높이 | 900px |
| → 처음 화면에 보이는 목록 | **61px** |
| 목록 안쪽 콘텐츠 높이 | **18,968px** |
| 페이지 전체 스크롤 여유 | 447px |

목록에 닿으려면 페이지를 447px 내려야 하는데, 그 구간에서 커서가 61px짜리 목록 위를 지나는
순간 **휠이 안쪽 목록에 붙잡힌다.** 스크롤 체이닝은 안쪽이 끝에 닿아야 바깥으로 넘어가므로,
사용자는 18,968px을 다 지나야 페이지가 움직인다 — 체감상 "페이지가 안 내려간다"이다.

**Task 030이 방아쇠였다.** 언론사가 17곳 → 34곳이 되며 한 실행의 기사 수가 두 배가 됐고,
안쪽 콘텐츠가 길어질수록 이 함정은 더 깊어진다.

**해소**: 목록의 자체 스크롤을 없애고 **페이지 스크롤 하나만** 쓴다. 목록은 내용만큼 자라고,
⑤ 미리보기를 `lg:sticky lg:top-20`으로 붙여 아래쪽 파일을 골라도 본문이 화면에 남게 했다.
실측 확인: 중첩 스크롤러 0개, 547행 페이지(21,447px)에서 맨 아래로 내려가 마지막 행
(`0547.txt`)을 클릭 → 미리보기가 그 기사 제목으로 갱신되고 sticky로 상단 80px에 붙어 있다.

**교훈 — 이것이 이 이슈를 남기는 이유다**: `scrollTop`이 끝까지 도달하는지만 보면 이 결함을
못 잡는다. **"사용자가 이 스크롤 영역에 닿으려면 먼저 페이지를 스크롤해야 하는가"**를 함께
물어야 한다. 세로로 쌓인 카드 아래에 놓인 영역에 자체 스크롤을 주는 것은 그 자체로 위험 신호다.

---

### I-060 · `latestSnapshotRef` 언마운트 동기화가 passive effect 플러시 타이밍에 의존한다

- 상태: 열림 (**기록만** · 재현 못 함 — 다음 회차 판단 자료)
- 발견: 23일차 · 저장소 계층(Task 031 교차검증 중)
- 관련 Task: Task 031

`app/results/page.tsx`·`app/keywords/page.tsx`는 "run이 바뀌면 렌더 도중 필터·결과를 초기화"하는 기존
패턴 때문에 그 필드들을 컨텍스트에 직접 바인딩하지 못하고(D-060), 로컬 state로 두었다가 **언마운트
시점에 한 번** 컨텍스트로 되돌려 쓴다.
...
}) // deps 없음 — 매 렌더 커밋 뒤 실행
useEffect(() => {
  return () => setResultsTab((prev) => ({ ...prev, ...latestSnapshotRef.current }))
}, []) // 언마운트 1회에만 실행
```

"최신 값 기록" effect는 `useEffect`(passive effect)라 브라우저 페인트 이후로 스케줄된다. 상태가 바뀐 바로
그 렌더의 기록 effect가 **실행되기 전에** 언마운트되면, cleanup은 이전 커밋의 낡은 스냅샷을 쓰고 가장
최근 변경분(예: 방금 입력한 검색어 한 글자)이 유실될 수 있다.

**재현하지 못했다.** 상태 변경(타이핑·필터 클릭)과 탭 이동(별도 클릭)이 서로 다른 이벤트라 그 사이에
React가 passive effect를 플러시할 시간이 있고, **상태 변경과 라우팅을 한 콜백에서 같이 하는 코드 경로가
이 앱에 없다**(교차검증에서 핸들러를 전수 확인). Playwright MCP로 반복한 모든 탭 왕복에서 유실 0건이었다.

**완화책(다음에 손댄다면)**: "최신 값 기록" effect를 `useLayoutEffect`로 바꾸면 커밋 직후 동기 실행되어
이 창이 닫힌다. DOM을 읽지 않는 순수 값 대입이라 레이아웃 스래싱 비용도 없다.

**다시 열어야 하는 조건**: 한 콜백 안에서 상태를 바꾸고 곧바로 `router.push`/`<Link>` 이동을 트리거하는
코드가 생기면 이 창이 실제로 열린다. 그때는 기록이 아니라 결함이다.
