...
- **구현 규칙**
  ...
  - 상태 7종(기본/일부 선택/진행 중/완료/부분 실패/언론사 0건/목록 로딩)을 모두 구현한다.
- **완료 조건 (DoD)**
...
- [ ] 대기 &nbsp;|&nbsp; 기능 ID: `F004` &nbsp;|&nbsp; 선행: Task 007, Task 015
...
- **완료 조건 (DoD)**
  - [ ] 실행 이력이 0건일 때 `GET /api/runs`가 에러가 아니라 빈 배열을 반환한다.
  - [ ] `GET /api/runs/{runId}/articles`가 기사 200건에서도 1초 내에 응답한다.
  - [ ] 삭제된 언론사가 포함된 과거 run을 조회해도 500이 나지 않는다.
  - [ ] Playwright MCP로 4개 엔드포인트를 순서대로 호출해 응답 스키마를 확인한다.
