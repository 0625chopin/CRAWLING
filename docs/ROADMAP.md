...
| **Phase 8** | 수집 결과 사용성과 표본 확충 (MVP 완료 후 신규 요청) `F001` `F004` `F007` | 2 (029–030) | 2 | ✅ 완료 |
| **Phase 9** | 탭 전환 상태 보존 (MVP 완료 후 신규 요청) 화면 셸 공통 | 1 (031) | 1 | ✅ 완료 |
| **합계** | | **31** | **31** | **100%** |
...
- **완료 조건 (DoD)**
  ...
  - [x] Playwright MCP로 "run 선택 → 분석 → 필터 조정 → 상투어 불용어 추가 → 재분석"을 태우고, 375px 폭에서 표가 카드 리스트로 바뀌는지 확인한다. — 전 동선 통과. 375px에서 표 `clientHeight === 0` · 카드 리스트 렌더 · `scrollWidth`(360) < `innerWidth`(375)로 **가로 스크롤 없음** 확인. 콘솔 에러 0건.
...
- **범위에서 빠진 것 (착수 전에 팀장이 이미 처리 완료)**
  ...
  - 위 4건은 Task 022 완료를 기다릴 이유가 없어 먼저 처리했다.
...
- [x] 완료 (15일차, 2026-08-11) &nbsp;|&nbsp; 기능 ID: — &nbsp;|&nbsp; 선행: Task 023
- **결과물**: `.env.example`(`CRAWL_PRESS_CONCURRENCY` 추가 — **I-032**) · `lib/crawler/config.ts`(결정 번호 D-013 → **D-015** 정정) · `lib/keyword/kiwi.ts`(`buildModelMissingMessage`에 `mkdir -p` 단계 추가 — 교차검증이 잡은 fail). **`README.md`·`docs/ROADMAP.md` §개발 환경 준비는 고치지 않았다** — 둘 다 개별 변수를 나열하지 않고 `.env.example`을 가리키기만 하므로 고칠 문장이 없는 것이 결론이다.
- **남긴 한계**: 없음. DoD ②의 "크롤 → 분석까지 도달"은 담당이 재현하지 못했으나 **같은 회차에 크롤 파이프라인이 실크롤 3회로 닫았다.**
...
- **완료 조건 (DoD)**
  - [x] `lib/crawler/config.ts`와 `lib/keyword/`가 읽는 환경변수가 `.env.example`에 **하나도 빠짐없이** 있고, 기본값이 코드와 일치한다. — 담당과 리뷰가 **각각 따로 전수 조사**해 7개로 일치했다. `process.env`를 읽는 파일은 `lib/crawler/config.ts` 하나이고 `lib/keyword/`는 환경변수를 전혀 읽지 않는다(모델 경로 하드코딩, 설계 의도). 빠져 있던 `CRAWL_PRESS_CONCURRENCY`를 추가했다(**I-032**).
  - [x] 새 머신에서 README만 보고 clone → 설치 → 실행 → 크롤 → 분석까지 도달할 수 있다. — 설치 명령·모델 URL·버전(`v0.23.0`)·파일 9개·스크립트 7종이 실물과 전부 일치하고 **막히는 문장이 0건**이었다. **크롤 → 분석 구간은 크롤 파이프라인이 실크롤로 닫았다** — 정상·중단·부분실패 3회를 태우고 부분 실패 run으로 `/results` → `[키워드 분석]` → `[분석 시작]`까지 이어 확인했다(기사 3건 · 토큰 423 → 279 · 고유 키워드 123, 콘솔 에러 0건).
  - [x] 모델 없이 분석을 시도했을 때 나오는 에러 메시지가 README의 해결 절차와 연결된다. — **교차검증이 fail을 잡아 닫았다.** 에러 메시지가 3단계인데 README는 4단계였고, 빠진 `mkdir -p data/kiwi-model` 때문에 **clone 직후 상태에서 명령을 그대로 복붙하면 마지막 `cp`가 목적지 부재로 실패**했다. 4단계로 맞추고 "이 목록은 README와 항상 같아야 한다"는 이유를 주석에 남겼다.

### Task 025 · MVP 전체 사용자 여정 수동 검증

- [x] 완료 (16일차, 2026-08-11) &nbsp;|&nbsp; 기능 ID: `F001`~`F008` 전체 &nbsp;|&nbsp; 선행: Task 024
- **참조**: `docs/PRD.md` §사용자 여정 1→5, 이 문서 §검증 시나리오
- **생성/수정 파일**: 없음 (검증 전용 Task — 발견된 결함은 해당 Task로 되돌려 수정)
- **결과물**: 새로 만든 파일 없음. 62개 항목 전수 통과 기록과, 검증이 잡아낸 결함 1건의 수정
  (`components/crawl/crawl-run-panel.tsx` · `docs/screens/01-crawl-run.md`, **I-038**·**I-041**).
- **남긴 한계**: **I-040**(부분 실패 Alert의 폴백 문구가 기사 단위 실패에서도 뜨는 좁은 경로) — 집계 단위를
  맞추려면 007·014A·016B의 계약을 동시에 건드려야 해 MVP가 안고 간다. 재검증에서 그 상태에 도달하지 못해
  실측되지 않았다는 사실도 함께 남긴다.
- **구현 규칙**
...
- **완료 조건 (DoD)**
  - [x] 검증 시나리오 **0~6단계 + 반응형·접근성 R-1~R-6**(분기 A·B 포함)이 모두 통과한다. — 62개 항목 전수 통과. 실제 언론사 3종 경로(블로터 RSS 요약 · 본문 전문 수집 ON · ZDNet 목록 페이지)를 한 실행에서 태웠다. 조사 제거는 `인텔은/인텔이/인텔`이 `인텔`(15회) 하나로 합산되는 것으로, 함정 ②는 `공개`(NNG 5회)·`경쟁`(NNG 3회) 생존으로 확인했다. **초판 DoD는 "1~5단계 + 분기 A·B"라고 적혀 있었다** — 실제 시나리오보다 좁아 0단계·6단계·반응형을 건너뛰고도 완료로 볼 수 있었다(**I-042**). 실제 범위로 정정했다.
  - [x] 데스크톱·모바일 양쪽에서 레이아웃이 깨지지 않는다. — 375/768/1280px 세 폭에서 5개 화면 순회. 375px 가로 스크롤 없음, 1280px 2단 레이아웃과 sticky 동작 확인.
  - [x] 브라우저 콘솔에 에러가 없다(`browser_console_messages`). — 전 구간 0건. **`all:true` 없이 마지막 네비게이션 기준으로 판정했다**(I-036) — `all:true`를 주면 세션 전체의 과거 에러를 자기 실패로 센다.
  - [x] `npm run lint` · `npm run typecheck` · `npm run test` · `npm run build` 모두 성공. — 팀장과 교차검증(저장소 계층)이 각각 독립 실행. vitest 18파일/192케이스, build 19개 라우트.
...
## Phase 8: 수집 결과 사용성과 표본 확충 ✅

> Phase 7과 같은 성격이다 — MVP 완료 후 사용자 요청으로 열린 확장이며 MVP 계획(SCHEDULE 244.6인일)에
> 포함되지 않는다. 22일차에 `/results`에서 겪은 불편 두 가지와 분석 정확도를 위한 표본 확충을 처리했다.

### Task 029 · 검색어 하이라이트와 기사 목록 스크롤 복구

- [x] 완료 (22일차, 2026-08-11) &nbsp;|&nbsp; 기능 ID: `F004` 확장 &nbsp;|&nbsp; 선행: Task 028
- **참조**: `docs/screens/02-collect-result.md` · `docs/CONVENTIONS.md` §8 §9
- **생성/수정 파일**: `lib/highlight.ts`(신규) · `lib/highlight.test.ts`(신규) ·
  `components/results/highlighted-text.tsx`(신규) · `components/results/{article-file-list,article-preview}.tsx` ·
  `app/results/page.tsx` · `app/globals.css` · `components/crawl/press-select-card.tsx` · `docs/screens/{01,02}-*.md`
- **결과물**: 검색어를 제목·본문 양쪽에서 `<mark>`로 칠한다(대소문자 무시 · 정규식 메타문자 14종 이스케이프).
  하이라이트 색은 `--highlight`/`--highlight-foreground` 토큰으로 두어 라이트·다크가 함께 따라온다(**D-058**).
  기사 목록 스크롤을 복구하고, 언론사가 34곳으로 늘며 드러난 카테고리 헤더 유실도 `sticky`로 함께 고쳤다.
- **구현 규칙**
  - ⚠️ **`dangerouslySetInnerHTML`을 쓰지 않는다.** 기사 본문은 외부에서 긁어온 문자열이라 그대로 XSS 경로다.
    문자열을 세그먼트 배열로 쪼개 React 노드로 렌더링한다.
  - ⚠️ **`ScrollArea`에 `max-h`만 주면 스크롤이 생기지 않는다**(**I-057**). Radix Viewport가 `size-full`이라
    부모 높이가 `max-height`뿐이면 `h-full`이 `auto`로 풀려 내용만큼 자라고, `max-h`는 잘라내기만 한다 —
    잘린 부분에 닿을 방법이 없다. `flex-1 min-h-0`으로도 안 풀린다. **`grid-rows-[minmax(0,1fr)]` + `min-h-0`**이
    답이다.
  - 색은 토큰으로만 쓴다. **다크 대비를 계산해 WCAG AA(4.5:1)를 넘겼는지 수치로 확인한다** — 하이라이트는
    읽으라고 칠하는 것이라 읽기 어려우면 목적을 잃는다.
- **완료 조건 (DoD)**
  - [x] 검색어가 제목·본문에서 칠해지고 대소문자를 무시한다. — Playwright 실측.
  - [x] 정규식 메타문자 14종이 이스케이프되고 테스트가 전부 덮는다. — 교차검증이 6/14만 덮던 것을 지적해 보강.
  - [x] 목록이 길 때 휠·스크롤바로 끝 항목까지 도달한다. — `scrollTop` 0→525/525, 모바일 1927/1927 실측.
  - [x] 다크 대비 4.5:1 이상. — 4.02:1 → **4.85:1**(팀장 독립 재계산으로 확인).
  - [x] `lint` · `typecheck` · `test` · `build` 통과.

### Task 030 · 언론사 표본 확충

- [x] 완료 (22일차, 2026-08-11) &nbsp;|&nbsp; 기능 ID: `F001` `F007` 확장 &nbsp;|&nbsp; 선행: Task 027
- **참조**: `docs/press-candidates.md`(실물 검증 자산 · §재현 방법) · `docs/ISSUES.md` **I-037**
- **생성/수정 파일**: `docs/press-candidates.md` · `data/press-sources.json` ·
  `lib/crawler/rss.ts` · `lib/crawler/rss.test.ts`
- **결과물**: 언론사 **17곳 → 34곳**(IT/AI 9 · 엔터 6 · 스포츠 7 · 경제 6 · 증권 6). 두드려 보고 제외한
  후보 22개를 사유별로 기록했다. 조사 중 `pubDate` **조용한 오파싱**을 발견해 함께 고쳤다(**D-056**) —
  노컷뉴스 계열이 월 이름 자리에 숫자를 찍어 `new Date()`가 8월 11일을 11월 7일로 읽었고,
  `Number.isNaN` 방어를 그대로 통과했다.
- **남긴 한계**: **I-054** — 표본이 두 배가 되며 완전 초기 상태의 손입력 부담도 17 → 34곳으로 늘었다.
  `press-defaults.ts` 시드는 I-037 설계대로 비운 채 두었다.
- **구현 규칙**
  - **실제로 두드려 본 것만 문서에 적는다.** 응답 코드·인코딩·아이템 수를 실측값으로 남기고, 죽은 후보는
    제외 사유와 함께 기록한다 — 그 기록이 다음 회차의 중복 조사를 막는다.
  - ⚠️ **item 레벨 `<description>`이 없는 피드는 등록하지 않는다** — 요약이 비어 개별 기사가 전부 실패한다.
  - ⚠️ **등록 전에 `&amp;(entity);` 이중 인코딩을 스캔한다**(**D-057**). 이중 인코딩 매체는 우리 파서가
    설계대로 한 번만 풀어도 `&quot;`가 남아 **키워드 랭킹을 오염시킨다**(**I-056** — `quot` 4위·`nbsp` 8위 실측).
  - ⚠️ **한글 등록은 파일 기반으로 한다** — 셸 경유로 넘기면 mojibake로 저장된다.
- **완료 조건 (DoD)**
  - [x] 각 카테고리가 6곳 이상이다. — `GET /api/press?category=…` 실측.
  - [x] 추가한 URL이 오늘 실측으로 살아 있고 값이 문서에 있다. — 교차검증이 5곳 재확인.
  - [x] 기존 언론사가 살아 있고 한글명에 mojibake가 없다. — U+FFFD 0건.
  - [x] 새 언론사로 실제 크롤이 돈다. — run `20260811-085056`(25건) · `20260811-092100`(10건).
  - [x] `lint` · `typecheck` · `test` 통과.

**Phase 8 완료 기준 (Exit Criteria)** — `/results`에서 검색어가 눈에 띄게 칠해지고 기사 목록을 끝까지
스크롤할 수 있다. 다섯 카테고리 모두 6곳 이상의 언론사를 갖고, 등록된 피드가 키워드 랭킹을 오염시키지 않는다.

---

## Phase 9: 탭 전환 상태 보존 ✅

> Phase 7·8과 같은 성격이다 — MVP 완료 후 사용자 요청으로 열린 확장이며 MVP 계획(SCHEDULE 244.6인일)에
> 포함되지 않는다. 23일차에 "상단 탭을 눌러도 다른 탭이 초기화되지 않게 해 달라"는 요청으로 열었다.

### Task 031 · 탭 전환 시 화면 상태 보존

- [x] 완료 (23일차, 2026-08-11) &nbsp;|&nbsp; 기능 ID: `F001`~`F008` 공통(화면 셸) &nbsp;|&nbsp; 선행: Task 029
- **참조**: `docs/screens/00-app-shell.md` · `docs/screens/README.md` · `docs/CONVENTIONS.md` §2 §5 §8
- **생성/수정 파일**: `components/app-state-provider.tsx`(신규) · `app/layout.tsx` ·
  `app/page.tsx` · `app/results/page.tsx` · `app/keywords/page.tsx` · `app/press/page.tsx` ·
  `app/stopwords/page.tsx`
- **결과물**: 탭 5개가 각자 라우트라서 떠나는 순간 언마운트되던 화면 상태를, 라우트 전환에도 살아남는
  앱 셸 위치로 올렸다. 되돌아오면 이전 화면을 그대로 그리고 데이터는 뒤에서 다시 불러온다(**D-059**).
  탭마다 컨텍스트를 따로 두어(`createTabStore` 팩토리) 한 탭의 입력이 다른 탭을 리렌더시키지 않는다.
  `hooks/` 아래 별도 접근 훅은 만들지 않았다 — 팩토리가 돌려주는 `useXTabField`가 그 역할을 한다.
- **남긴 한계**: `/results`·`/keywords`는 "run이 바뀌면 렌더 도중 초기화"하는 기존 패턴 때문에 일부 필드를
  로컬 state로 두고 언마운트 시점에 되돌려 쓴다(**D-060**). 그 패턴의 이론적 취약점은 **I-060**에 기록만
  해 두었다(재현 실패). 새로고침(F5) 지속은 범위 밖 그대로다 — 이번 구현은 순수 인메모리다.
- **구현 규칙**
  - 프로바이더는 `app/layout.tsx`의 `ThemeProvider` **안쪽**, `{children}`을 감싸는 위치에 둔다.
    루트 레이아웃은 라우트가 바뀌어도 다시 마운트되지 않으므로 여기 올린 상태만 살아남는다.
  - ⚠️ **캐시 무효화 규칙을 만들지 않는다.** "복귀 시 이전 데이터를 그대로 그린 뒤 뒤에서 재조회"
    (stale-while-revalidate)로 통일한다. 어느 탭의 변경이 어느 탭의 캐시를 깨는지 표를 만들기 시작하면
    작업량이 배로 늘고, 표에서 빠진 경로가 그대로 "화면은 멀쩡한데 값이 낡은" 결함이 된다.
    스켈레톤은 **보관된 데이터가 없을 때만** 띄운다.
  - ⚠️ **컨텍스트 `value`를 매 렌더 새 객체로 만들지 않는다.** 상태 40개를 객체 하나에 담아 내려주면
    한 탭의 입력 한 글자가 나머지 네 탭을 전부 리렌더시킨다. 탭별로 컨텍스트를 나누거나 `useMemo`로
    참조를 고정한다. **이 실수는 화면이 멀쩡해 보이므로 리뷰에서만 잡힌다.**
  - **상태 관리 라이브러리를 도입하지 않는다**(zustand·jotai·react-query 등). React Context로 끝낸다 —
    `package.json` 변경은 회차 단위로 팀장이 처리한다(`docs/CONVENTIONS.md` §8).
  - **일시적 UI 상태는 보존 대상이 아니다.** 다이얼로그 열림, 삭제 확인 대상, 토스트, 로딩 플래그는
    탭을 떠나면 닫혀 있는 것이 맞다. 보존하는 것은 **사용자가 손으로 만든 선택**(고른 언론사, 고른 실행,
    검색어, 필터값)과 **다시 받아오면 되는 데이터의 직전 스냅샷**이다.
  - 진행 중 크롤의 `runId`를 보존해 탭을 다녀와도 진행률이 이어지게 한다. 크롤 잡은 `globalThis`
    레지스트리에서 계속 돌고 있으므로(Q3) 화면이 `runId`만 기억하면 폴링이 다시 붙는다.
  - **페이지의 마크업과 로직 구조는 바꾸지 않는다.** 상태 선언부와 조회 이펙트의 조건만 바뀐다.
    설계서에 없는 UI를 새로 만들지 않는다.
  - 새로고침(F5)까지 살리는 `sessionStorage` 지속은 **이번 범위 밖**이다. 필요해지면 별도 Task로 연다.
- **완료 조건 (DoD)**
  - [x] `/`에서 언론사를 몇 곳 고르고 최대 기사 수를 바꾼 뒤 다른 탭에 갔다 돌아오면 선택과 입력값이 그대로다.
        — 담당 실측 "2/34개 선택됨 · 최대 7", 교차검증 독립 실측 "2/34개 선택됨 · 최대 5".
  - [x] 크롤 실행 중 다른 탭으로 이동했다 돌아오면 진행률이 **이어서** 보인다(0으로 초기화되지도, 떠난 시점
        값에 멈춰 있지도 않다). — 교차검증 실측: 34곳 크롤 중 `/keywords` 왕복 후 "전체 진행률 93% ·
        현재: ZDNet 코리아 18/20건". 담당 실측도 41% 시점 이탈 → 복귀 시 서버 현재 상태(완료) 반영.
  - [x] `/results`의 실행 선택·검색어·선택 기사, `/keywords`의 실행 선택·필터 4종·분석 결과,
        `/press`의 카테고리 필터, `/stopwords`의 검색어가 탭 왕복 뒤에도 남아 있다. — 양쪽 실측 일치.
        `/keywords`는 **재분석 없이** 결과가 그대로 남는 것까지 확인.
  - [x] 복귀할 때 스켈레톤이 다시 뜨지 않는다. 대신 목록이 뒤에서 갱신된다. — 보관 스냅샷이 있으면
        `useState(() => snapshot !== null ? 'ready' : 'loading')`로 초기 상태를 건너뛴다.
  - [x] `/press`에서 언론사를 추가하고 `/`로 이동하면 새 언론사가 목록에 보인다(재조회가 실제로 돈다).
        — 담당 실측 약 800ms 뒤 반영, 교차검증 실측 "34/35개 선택됨". 양쪽 다 테스트 언론사를 지워 원복.
  - [x] 다이얼로그 열림 같은 일시적 UI 상태는 보존되지 **않는다**. — 컨텍스트에 올리지 않아 설계로 보장.
  - [x] Playwright MCP로 탭 5개 왕복 동선을 태우고 콘솔 에러 0건을 확인한다. — 담당·교차검증 양쪽 0건
        (세션 전체가 아니라 마지막 네비게이션 기준으로 판정 — I-036).
  - [x] `npm run lint` · `typecheck` · `test` · `build` 통과. — 팀장 재실행: 0건 · 0건 ·
        24파일 299케이스 · 빌드 성공(정적 페이지 13개).

**Phase 9 완료 기준 (Exit Criteria)** — 상단 탭을 어떤 순서로 눌러도 앞서 하던 작업이 남아 있고,
크롤을 돌려 둔 채 다른 탭을 둘러보고 와도 진행 상황을 놓치지 않는다.
...
| 5-3 | 일괄 추가 패널 펼쳐 `특파원, 인턴기자\n논설위원` 입력 | 3건으로 분리되어 그대로 추가됨 (콤마·줄바꿈 혼합 파싱). **예시 단어는 기본 불용어 7건과 겹치지 않는 것으로 고른다** — 겹치면 "N개 추가·1개 중복"이 나와 정상 동작이 결함처럼 보인다(I-039) |
