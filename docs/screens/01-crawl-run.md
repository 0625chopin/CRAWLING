...
#### 언론사 단위 실패가 0곳인데도 이 상태에 들어오는 경우 (I-040, 20일차 반영)

run 레벨 상태(`partial-failed`/`failed`)를 정하는 것은 **기사 단위** `failCount`(`lib/storage/run-repository.ts`의 `finishRun`)이고, 위 요약·리스트가 강조하는 "N개 언론사 실패"는 언론사 전체가 피드·목록 페이지조차 열지 못한 **언론사 단위** 실패(`pressStatuses[].status === 'failed'`)다. 한 언론사 안에서 기사 몇 건만 개별적으로 실패하고 나머지는 저장에 성공하면, 그 언론사는 `done`으로 끝나는데도 run은 이 상태(⑤)로 들어온다 — 이때 언론사 단위 실패는 0곳이다.

이 경우(`failedPresses.length === 0`)에는 "언론사가 실패했다"는 말을 쓰지 않는다. 언론사 목록은 전부 완료로 보이기 때문이다.

- 요약줄: `{N}개 언론사 · 기사 {successCount}건 저장 · {failCount}건 개별 실패`
- Alert 제목: `기사 {failCount}건 개별 수집 실패`
- Alert 본문: `언론사는 모두 정상 처리됐지만, 개별 기사 {failCount}건이 수집에 실패했습니다.`

**근본 원인은 그대로 남아 있다** — 집계 단위 자체(기사 단위 vs 언론사 단위)가 어긋난 것이라, 근본 해소는 `finishRun`·`RunProgress` 스키마를 건드려야 하고 MVP 범위 밖이다(`docs/ISSUES.md` I-040). 이번 반영은 **문구가 실제로 일어난 일(기사 단위 실패)과 다른 말(언론사 단위 실패)을 하지 않도록 정정한 것**뿐이다.
