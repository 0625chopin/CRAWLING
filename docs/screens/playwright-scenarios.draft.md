...
**13일차(2026-08-11) 갱신** — 3일차 최초 작성 이후 009A/009B·012A/012B·016A·018A·018B가 전부 완료됐다(`docs/ROADMAP/work/03.화면.md`
담당 Task 11건 중 화면 자체를 만드는 7개 조각은 이제 전부 완료 상태). 이번 갱신은 **016A 절의 "이 절이 다루지 않는 것"** —
016B(크롤 파이프라인, 진행·완료·부분 실패·중단 전환)가 그 사이 완료되어 016A 단독으로는 볼 수 없던 DoD 항목도 지금은 화면에서
직접 확인할 수 있다는 점을 반영한다. 나머지 섹션(009A/B·012A/B·018A/B)의 시나리오 본문은 실물과 대조해도 어긋난 곳이 없어
그대로 둔다. 실제 개별 시나리오 실행(브라우저로 태우는 일)은 025 사전 리허설 용도로 `docs/screens/verification-checklist.draft.md`에
더 최신 상태로 정리해 두었다 — 이 문서는 여전히 **Task 단위 DoD 대조**가 목적이고, 그 문서는 **여정(사용자 journey) 단위 통과
여부**가 목적이라 서로 보완 관계다.
...
| 언론사 2곳 체크→실행→진행률 갱신 | **O(13일차 갱신)** | "2곳 체크"로 버튼이 `disabled`→활성으로 전환되는 것은 016A. 클릭 이후 실행·진행률 갱신은 016B 소관이었으나 **11일차에 016B가 완료되어 지금은 화면에서 직접 확인 가능** — 아래 "016B 완료 반영" 절 참고 |
| 완료 후 `[수집 결과 보기]`→`/results`, txt 저장 확인 | **O(13일차 갱신)** | 016B 완료 — `crawl-run-panel.tsx`의 `DonePanel`이 `link "수집 결과 보기"`를 렌더링 |
| 중단 버튼 | **O(13일차 갱신)** | 016B 완료 — `RunningPanel`의 `button "중단"`, `abortCrawl` API 연결 확인됨 |
...
이어붙이는 부분)은 016B 완료 후 크롤 파이프라인 워크스트림이 별도 시나리오로 검증하는 것이 원안이었다.

### 016B 완료 반영 (13일차) — 진행·완료·중단 전환

016B는 11일차에 완료됐다(`docs/ROADMAP/work` 크롤 파이프라인 영역 소관, 이 워크스트림 담당 Task는 아니다). 이 절의 시나리오
8번에서 멈췄던 지점을 이어서, **참고용으로만** 이어지는 흐름을 적어 둔다(정식 검증은 크롤 파이프라인 워크스트림 몫이고, 이
문서가 소유권을 가져오는 것은 아니다):

9. 8번에서 `button "크롤링 시작"` 클릭 → 버튼이 `button "중단"`으로 전환, `role="status" aria-live="polite"` 영역에 전체
   진행률(`Progress`)과 "현재: {언론사} — n/N건" 텍스트가 1초 간격으로 갱신되는지 확인(`use-crawl-progress.ts`가 폴링).
10. `PressRunStatusList`에 언론사별 상태(`대기`/`진행중`/`완료`/`실패`)가 아이콘 + 텍스트로 함께 표시되는지(색상 단독 아님).
11. 완료 대기 → `DonePanel` 헤딩("크롤링 완료"/"크롤링 완료 (일부 실패)")과 요약, `link "수집 결과 보기"`(`/results`) +
    `button "새로 크롤링하기"`(runId를 null로 되돌려 idle 상태로 복귀) 확인.
12. 진행 중 `button "중단"` 클릭(별도 실행으로 재현) → 상태가 `aborted`로 종료되고 `DonePanel`이 "크롤링 중단됨" 헤딩 +
    `Square` 아이콘으로 전환되는지(D-030 결정 1 — 완료 조각을 재사용하되 아이콘·문구·톤만 갈아 끼운다).

전체 여정(언론사 등록 → 실행 → 결과 확인)을 하나로 잇는 상세 체크리스트는 `docs/screens/verification-checklist.draft.md`
§2단계·§6단계에 더 최신 상태로 정리돼 있다 — 이 절은 그 문서의 요약판이다.
...
| --- | --- | --- | --- |
| 009A+009B (Task 009 전체) | 7 | 6 (①②③④⑥⑦) + ⑤ 13일차 갱신 — 016A가 완료돼 "활성→홈 반영"도 코드상 확인 가능해졌다(위 016A 절 시나리오 4번) | 0개(형식상 이월 표기는 남아 있으나 013·016A 완료로 실질적으로는 열렸다) |
| 012A+012B (Task 012 전체) | 5 | 5 (전부) | 없음 |
| 016A (Task 016 중 이 워크스트림 담당분) | 5 | 5(13일차 갱신) | 없음 — ①②는 016A 자체 커버, ③④⑤(진행률 갱신·완료 이동·중단)는 016B 소관 컴포넌트였으나 11일차에 016B가 완료되어 화면에서 직접 확인 가능해졌다(위 "016B 완료 반영" 절 참고). **다만 016B 자체는 여전히 이 워크스트림의 담당 Task가 아니다** — 정식 검증 책임은 크롤 파이프라인 워크스트림에 있고, 이 문서는 참고용 흐름만 남겼다 |
...
- **13일차 관찰**: 이번 갱신 작업 중 Playwright MCP로 `/`·`/press`·`/stopwords`·`/results`를 열어 실물과 대조하려 했으나,
  개발 서버가 일시적으로 `/`만 (다른, 더 오래된 스캐폴드로 보이는) 200을 반환하고 나머지 라우트는 전부 404를 반환하는 상태였다
  (다른 워크스트림이 같은 저장소에 동시에 파일을 쓰고 있어 생긴 것으로 추정, 수 차례 재시도해도 동일 — 서버 재시작은 지시에 따라
  하지 않았다). 이번 갱신의 근거는 전부 소스 코드 직접 읽기다. 자세한 내용은
  `docs/screens/verification-checklist.draft.md` §0 "라이브 브라우저 확인의 제약" 참고.
