...
### 공통 관찰 — A/B 파일 경계와 브라우저 도달성

여러 화면 Task가 10인일을 넘어 `A`/`B`로 쪼개졌는데(`docs/ROADMAP/work/03.화면.md`), **쪼갠 두 조각이 같은 파일을 함께 완성해야
라우트가 실제로 보이는 경우**가 있다. 이 경우 앞 조각(A)만 완료된 시점에는 Playwright MCP로 그 조각 자신의 DoD조차 확인할 방법이
없다. 아래 표로 정리한다(상세 근거는 `docs/ISSUES.draft.화면.md` 참고).

| Task | `app/**/page.tsx` 소유 | page.tsx가 참조해야 하는 다른 조각 소유 | 결과 |
| --- | --- | --- | --- |
| 009A/009B | **009B** | — | 009A는 표/카드 컴포넌트만 만들고 라우트에 붙이지 않는다. **009A 단독 시점에는 `/press`가 여전히 ScreenPlaceholder.** |
| 012A/012B | **012A** | `stopword-add-card.tsx`(012B) | 012A가 page.tsx를 미리 정리해 두므로 012A 단독으로도 목록/섹션 부분은 라우트에서 확인 가능 |
| 016A/016B | **016A** (016B는 크롤 파이프라인 소관) | `crawl-run-panel.tsx`(016B, 다른 워크스트림) | 016A 단독 시점에는 우측 실행 패널의 실제 모습이 불확실(문서 간 불일치, 아래 이슈 참고) |
| 018A/018B | **018A** | `article-file-list.tsx`·`article-preview.tsx`(018B) | 018A 단독으로는 마스터-디테일 우측 절반이 비어 있을 가능성이 큼 |
...
**전제 조건**: Task 011(불용어 CRUD API) 완료. 이 조각은 `app/stopwords/page.tsx`를 직접 정리하므로(공통 관찰 표 참고),
...
**전제 조건**: Task 015A(크롤 API), Task 009A(`source-type-badge.tsx`) 완료. **주의**: 우측 실행 패널
(`crawl-run-panel.tsx`)은 016B(크롤 파이프라인) 소관이라, 016A 완료 시점에 우측 패널의 실제 모습이 두 문서 사이에서
불명확하다(`docs/ISSUES.draft.화면.md` 참고). 이 절은 **016A가 자체적으로 검증 가능한 좌측 언론사 선택 카드**에 한정한다.
...
| 언론사 0건 → `Inbox` 빈 상태 + `[언론사 관리로 이동]` + 실행 버튼 비활성 | **부분** | `Inbox` 빈 상태는 O. "실행 버튼 비활성"은 우측 패널 소유가 불명확해 016A 시점 확정 불가 |
| 언론사 2곳 체크→실행→진행률 갱신 | X | 016B(크롤 파이프라인) — 이 워크스트림 시나리오 밖 |
...
7. `[전체 해제]` 클릭 → 모두 해제, 선택 0개일 때 버튼 자체가 `disabled` 확인.
8. 각 행의 방식 배지(RSS/HTML) 텍스트와 `sourceUrl`(`font-mono`) 노출 확인. `source-type-badge.tsx`(009A 산출물)
   재사용 여부는 코드 리뷰로 별도 확인한다(브라우저로는 렌더링 결과만 확인 가능).
9. `browser_resize(1280, 800)` → `lg:grid-cols-[1fr_360px]` 2단 레이아웃, 우측 카드가 `sticky`로 유지되는지 스크롤 후
   `browser_evaluate`(`getBoundingClientRect`)로 위치 비교.
10. `browser_resize(768, 1024)`, `browser_resize(375, 812)` → 1단 스택(목록 → 실행 패널), 375px에서 언론사 목록
    `ScrollArea` 높이가 `h-[320px]`로 축소되는지 확인.
11. **언론사 0건**: `data/press-sources.json`을 빈 배열로 교체 → 재방문 → `Inbox` 아이콘 + "등록된 언론사가 없습니다" +
    `[언론사 관리로 이동]` → 클릭 → `/press`로 이동 확인.
12. 로딩 스켈레톤: API 응답 지연 재현이 가능하면(`browser_network_request` 등) `Skeleton` 5개 노출을 확인.
...
우측 실행 패널(진행률/중단/완료/부분 실패)은 016B 완료 후 크롤 파이프라인 워크스트림이 별도 시나리오로 검증한다.

---

## Task 018A · 수집 결과 화면 — 실행 선택 및 요약 카드

**전제 조건**: Task 017(수집 결과 API) 완료. **주의**: `article-file-list.tsx`·`article-preview.tsx`는 018B 소관이라
(공통 관찰 표 참고), 018A 완료 시점 마스터-디테일 레이아웃의 좌/우 절반이 비어 있을 수 있다. 이 절은 018A 자체 DoD에 한정한다.
...
| 016A (Task 016 중 이 워크스트림 담당분) | 5 | 1.5 (①, ②의 빈 상태 부분) | 3.5개 — 진행률/완료 이동/중단/②의 "실행 버튼 비활성"은 016B(크롤 파이프라인) 소관 컴포넌트가 필요해 이 워크스트림 시나리오 밖 |
...
- 016A·018A는 "자기 조각만으로는 라우트에 온전히 도달할 수 없다"는 구조적 제약이 있다(공통 관찰 표). 해당 Task가
  실제로 열리는 회차의 담당자는 이 문서의 "이 절에서 다룸" 표를 먼저 확인하고, 짝이 되는 B 조각이 끝나기 전까지는
  이 문서의 해당 단계를 스킵해야 한다.
- 발견한 설계서 ↔ ROADMAP DoD 불일치는 `docs/ISSUES.draft.화면.md`에 남겼다(009A/009B 파일 경계, Task 016 실행 패널
  소유 불일치, 012A/012B의 `stopword-add-card.tsx` 배선 주체 불명확, 018A/018B의 우측 패널 초기 모습 미정의).
