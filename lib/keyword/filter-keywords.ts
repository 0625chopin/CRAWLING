 ...
 * 다시 정렬하지 않는다.
 *
 * 항목 타입을 제네릭으로 둔 것은 소비자가 둘이기 때문이다: run 단위 `KeywordCount`와 시간대
 * 단위 `DailyKeywordItem`(직전 구간 대비 증감이 붙은 형태). 필터 규칙은 `posTag`·`count`만 보므로
 * 같은 함수로 충분하고, 타입마다 복사해 두면 minCount 경계 같은 규칙이 한쪽만 바뀐다.
 */
export function filterKeywordItems<T extends { posTag: PosTag; count: number }>(
  items: readonly T[],
